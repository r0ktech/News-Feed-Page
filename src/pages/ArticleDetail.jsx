...
function ArticleDetail() {
        ...
        if (storedArticle) {
          ...
          if (response.data.articles) {
            const related = response.data.articles
              .filter(a => a.title !== parsedArticle.title && a.title !== '[Removed]')
              .slice(0, 2)
  ...
  const getCategoryFromArticle = (article) => {
    ...
    if (title.includes('tech') || description.includes('tech') || title.includes('ai') || description.includes('ai') || content.includes('tech') || content.includes('ai')) {
      return 'Technology'
    }
    if (title.includes('business') || description.includes('business') || content.includes('business')) {
      return 'Business'
    }
    if (title.includes('sport') || description.includes('sport') || content.includes('sport')) {
      return 'Sports'
    }
    if (title.includes('art') || description.includes('art') || content.includes('art') || title.includes('culture') || description.includes('culture')) {
      return 'Arts'
    }
    if (title.includes('world') || description.includes('world') || content.includes('world')) {
      return 'World'
    }
    return 'Technology'
  }
  ...
  const authorName = article.author?.replace(/^.*?\|/, '').trim() || article.author || 'Amelia Harper'
    ...
    <div className="article-detail-page">
      ...
      <div className="article-detail-container">
        ...
        <div className="article-main-image">
          ...
          <div className="article-image-placeholder" style={{ display: article.urlToImage ? 'none' : 'block' }}></div>
        </div>
        ...
        <div className="article-body">
          {article.content ? (
            <>
              ...
              <p>{article.content.replace(/\[.*?\]/g, '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().substring(0, 2000)}...</p>
            </>
          ) : (
            ...
            {relatedArticles.length > 0 ? (
              relatedArticles.map((related, index) => (
                <div key={index} className="related-article-card">
                  ...
                  <div className="related-article-image">
                    <img 
                      src={related.urlToImage || 'https://via.placeholder.com/200x200?text=News'} 
                      alt={related.title}
                      onError={(e) => {
                  ...
                  <a 
                    ...
                    onClick={(e) => {
                      e.preventDefault()
                      localStorage.setItem(`article_${related.title}`, JSON.stringify(related))
              ...
              <>
                <div className="related-article-card">
                  <div className="related-article-category">{category}</div>
                  <h3 className="related-article-title">The Future of AI: Trends and Predictions</h3>
                  <p className="related-article-description">
                  ...
                  <a href="#" className="related-article-link">Read More</a>
                </div>
                <div className="related-article-card">
                  <div className="related-article-category">{category}</div>
                  <h3 className="related-article-title">Sustainable Tech: Innovations for a Greener Future</h3>
                  <p className="related-article-description">
